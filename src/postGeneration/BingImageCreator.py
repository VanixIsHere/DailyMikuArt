## ORIGINAL SOURCE: https://github.com/acheong08/BingImageCreator/
...
class ImageGen:
    ...
    def get_images(self, prompt: str) -> list:
        ...
        if response.status_code != 302:
            ...
            response = self.session.post(url, allow_redirects=False, timeout=5000)
            if response.status_code != 302 and response.status_code != 200:
                if self.debug_file:
                    self.debug(f"ERROR: {error_redirect}")
                # print(f"ERROR: {response.text}")
                print(f"Bing error with code: ${response.status_code}")
...
class ImageGenAsync:
    ...
    async def get_images(self, prompt: str) -> list:
        ...
        if response.status_code != 302:
            ...
            if response.status_code != 302:
                # print(f"ERROR: {response.text}")
                print(f'ERROR redirecting Bing with status code: {response.status_code}')
