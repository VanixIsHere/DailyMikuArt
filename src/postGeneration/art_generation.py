...
def inscribe_text_on_images(props: defs.PostProps, text: str, image_file_list: list[str]):
    for image in image_file_list:
        try:
            ...
            font_loc = os.path.join(defs.ROOT_DIR, 'postResources', 'BrushHandNew.ttf')
            font_size = 32
            font_leading = 8
            x = 30
            y = 30
    ...
    if answer == "yes" or answer == 'y':
        ...
        try:
            ...
            try:
                ...
                stability_image_file = os.path.join(props.folderName, '{uuid}_stab_miku_art_{date}_{attempt}.png'.format(uuid=props.uuid, date=props.date, attempt=props.attempt))
                ...
                try:
                    ...
                    dalle_image_file = os.path.join(props.folderName, '{uuid}_dalle_miku_art_{date}_{attempt}.png'.format(uuid=props.uuid, date=props.date, attempt=props.attempt))
